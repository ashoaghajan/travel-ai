/**
 * The only module in the app that touches the key-value store.
 *
 ...
 *
 * **A copy of `src/services/localStorage.service.ts`, differing in two
 * places** — both marked below, both forced by the platform. `tokens.test.ts`
 * guards the design system against drift the same way; this file is guarded by
 * `core-copies.test.ts`, which diffs it against the web's and allows exactly
 * the two hunks it knows about.
 */

import { mmkvStorage } from './mmkvStorage';
import type { StorageLike } from './mmkvStorage';
...
export const STORAGE_KEYS = {
  ...
  /**
   * Whether the messages panel was left open. Device-scoped UI position, like
...
/*
 * DIFFERS FROM WEB (1/2): the backing store.
 *
 * Never null here. The web has to allow for `localStorage` throwing outright —
 * private mode, blocked cookies — and every caller already handles the empty
 * result that produces. MMKV has no such failure, so the null branch stays in
 * the type for the shared code path and is simply never taken.
 */
...
/*
 * DIFFERS FROM WEB (2/2): no cross-process listener.
 *
 * The web subscribes to `storage` events so a write in one tab reaches the
 * others. A phone has one process and no second tab, so there is nothing to
 * listen to — `notify` from `set`/`remove` is the whole story. Deleted rather
 * than stubbed, because a no-op listener would imply a synchronisation that
 * does not exist.
 */
