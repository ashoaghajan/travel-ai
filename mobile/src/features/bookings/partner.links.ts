...
import { formatDateRange, formatShortDate } from '../../core/utils/date';
 ...
 *
 * DIFFERS FROM WEB: `process.env.EXPO_PUBLIC_*` rather than `import.meta.env`.
 * Metro's Babel cannot *parse* `import.meta`, so the web's form is a build
 * error here rather than a runtime one — the same reason `http.ts` gives for
 * its base URL. The names change with the prefix, which means the ids have to
 * be set twice; that is the cost of the two apps having different bundlers,
 * and an unset id is already handled — it falls back to the partner's home
 * page rather than building a half-filled search.
 */
function affiliateId(builder: PartnerLinkBuilderId): string {
  switch (builder) {
    case 'expedia':
      return process.env.EXPO_PUBLIC_EXPEDIA_AFFILIATE_ID ?? '';
    case 'booking':
      return process.env.EXPO_PUBLIC_BOOKING_AFFILIATE_ID ?? '';
    case 'trip':
      return process.env.EXPO_PUBLIC_TRIP_AFFILIATE_ID ?? '';
    case 'getyourguide':
      return process.env.EXPO_PUBLIC_GETYOURGUIDE_AFFILIATE_ID ?? '';
  }
}
